...
\begin{center}
{\huge SMO  1st Round} \\
\medskip Lausanne, Zürich - January 14, 2006
\end{center}
\vspace{8mm}
...
\item[\textbf{1.}] Find all triples $(p,q,r)$ of prime numbers such that their pairwise differences
\[|p-q|,\quad |q-r|,\quad |r-p|\]
are also prime numbers.

\bigskip

\item[\textbf{2.}] Let $n$ be a positive integer. Determine the number of subsets $A \subset \{1,2,\ldots,2n\}$
with the property that there are not two elements $x,y \in A$ with $x+y = 2n+1$.

\bigskip

\item[\textbf{3.}] In triangle $ABC$ define $D$ as the intersection of the angle bisector of $\wi BAC$
with the side $BC$. Assume that the circumcenter of triangle $ABC$ coincides with the incenter of triangle
$ADC$. Determine the angles of $\triangle ABC$.

\bigskip

\item[\textbf{4.}] Find all positive integer solutions to the equation
\[\lcm(a,b,c)=a+b+c.\]

\bigskip

\item[\textbf{5.}] An $m \times n$-board is divided into unit squares. An L-triomino consists of three unit
